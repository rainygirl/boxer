<script lang="ts">
  ...
  async function reload() {
    tasks = await tasksApi.byUser($page.params.userId);
  }
  ...
  const SORT_KEY = 'boxer:member-issues-sort';
  ...
  // Table collapse
  const COLLAPSE_KEY = 'boxer:member-issues-collapse';
      ...
      {#if targetUser}
        <div class="w-6 h-6 rounded-full overflow-hidden shrink-0">
          {#if targetUser.avatar_url}
            <img src={targetUser.avatar_url} class="w-full h-full object-cover" alt={targetUser.name} />
          {:else}
            <div class="w-full h-full bg-brand-400 text-white text-[10px] flex items-center justify-center font-semibold">
              {targetUser.name?.[0] ?? '?'}
            </div>
          {/if}
        </div>
    ...
    <div class="flex-1 flex gap-3 overflow-x-auto overflow-y-hidden p-4">
      {#each TASK_STATUSES as statusCfg}
        ...
        <div class="flex flex-col w-64 shrink-0 rounded-xl bg-slate-50 dark:bg-slate-800/50 overflow-hidden">
          ...
          <!-- Cards -->
          <div
            use:dndzone={{ items: colItems, type: 'member-issues-board', flipDurationMs: FLIP_MS, dropTargetStyle: {} }}
  ...
  {:else}
    ...
    <div class="flex flex-col {narrow ? '' : 'min-w-[580px]'}">
      ...
      {#each TASK_STATUSES as statusCfg}
        ...
        <!-- Group rows -->
        <div
          use:dndzone={{ items: groupItems, type: 'member-issues-table', flipDurationMs: FLIP_MS, dropTargetStyle: { outline: 'none', background: 'rgba(99,102,241,0.04)' } }}
      ...
      {#if tasks.length === 0}
        <div class="flex flex-col items-center justify-center py-20 text-slate-400 dark:text-slate-600">
          <p class="text-sm">{targetUser ? `${targetUser.name}에게 할당된 이슈가 없습니다.` : '이슈가 없습니다.'}</p>
        </div>
      {/if}
    </div>
    </div>
